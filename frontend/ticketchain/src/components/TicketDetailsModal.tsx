            ...
            <div className="rounded-2xl border border-border/60 bg-background/70 px-4 py-4">
              <div className="flex items-center justify-between">
                <button
                  ...
                  disabled={isLoadingMetadata || !imageUrl}
                  className="inline-flex items-center gap-2 text-sm font-semibold text-primary transition hover:text-accent disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {showImage && imageUrl && !isLoadingMetadata && (
              ...
              {!imageUrl && !isLoadingMetadata && (
                <div className="mt-4 rounded-lg border border-border/60 bg-muted/50 px-4 py-3 text-center text-sm text-muted-foreground">
                  No NFT image found in metadata
                </div>
              )}
            </div>
            ...
            <div className="space-y-2 text-sm">
              {metadataJson.name && (
                <div>
                  ...
                  <span className="text-muted-foreground">{metadataJson.name}</span>
                </div>
              )}
              {metadataJson.description && (
                <div>
                  <span className="font-medium text-foreground">Description: </span>
                  <span className="text-muted-foreground">{metadataJson.description}</span>
                </div>
              )}
              {metadataJson.attributes && Array.isArray(metadataJson.attributes) && (
                <div>
                  <span className="font-medium text-foreground">Attributes: </span>
                  <div className="mt-1 flex flex-wrap gap-2">
                    {metadataJson.attributes.map((attr: any, index: number) => (
                      <span
                        key={index}
