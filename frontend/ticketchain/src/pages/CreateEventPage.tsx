...
import { useCreateEvent, useAddTicketType, useEventCounter } from '../hooks/useContracts';
...
export const CreateEventPage = () => {
  ...
  const handleTicketTypeChange = (index: number, field: keyof TicketTypeInput, value: string) => {
  ...
  const addTicketTypeField = () => {
    setTicketTypes(prev => [...prev, { name: '', price: BigInt(0), supply: BigInt(0) }]);
  };
  ...
  const validateForm = (): boolean => {
    ...
    const totalTicketSupply = ticketTypes.reduce((sum, type) => sum + Number(type.supply), 0);
    if (totalTicketSupply !== parseInt(formData.totalSupply)) {
      newErrors.totalSupply = 'Total supply must equal sum of all ticket type supplies';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const startTime = BigInt(Math.floor(new Date(`${formData.startDate}T${formData.startTime}`).getTime() / 1000));
      const endTime = BigInt(Math.floor(new Date(`${formData.endDate}T${formData.endTime}`).getTime() / 1000));
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Create a New Event</h1>
          <p className="text-gray-600">Fill out the form below to create your event on TicketChain.</p>
        </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      royaltyBps: (parseFloat(e.target.value) * 100).toString()
                    }))}
                    min="0"
                    max="10"
                    step="0.1"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                              errors[`ticketType${index}Name`] ? 'border-red-500' : 'border-gray-300'
                            }`}
                            placeholder="e.g., VIP, General"
                          />
                          {errors[`ticketType${index}Name`] && (
                            <p className="text-red-500 text-sm mt-1">{errors[`ticketType${index}Name`]}</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handleTicketTypeChange(index, 'price', (parseFloat(e.target.value || '0') * 1e18).toString())}
                            min="0"
                            step="0.001"
                            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                              errors[`ticketType${index}Price`] ? 'border-red-500' : 'border-gray-300'
                            }`}
                            placeholder="10"
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Price in PC (Push Chain tokens). Users can pay with any supported currency.
                          </p>
                          {errors[`ticketType${index}Price`] && (
                            <p className="text-red-500 text-sm mt-1">{errors[`ticketType${index}Price`]}</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handleTicketTypeChange(index, 'supply', e.target.value)}
                            min="1"
                            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                              errors[`ticketType${index}Supply`] ? 'border-red-500' : 'border-gray-300'
                            }`}
                            placeholder="100"
                          />
                          {errors[`ticketType${index}Supply`] && (
                            <p className="text-red-500 text-sm mt-1">{errors[`ticketType${index}Supply`]}</p>
                          )}
                        </div>
                      </div>
                    </div>
